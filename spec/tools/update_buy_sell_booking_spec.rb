...
RSpec.describe Tools::UpdateBuySellBooking do
  describe ".call" do
    let(:valid_params) do
      {
        "bookyear_id" => 1,
    ...
    it "updates a booking successfully" do
      ...
      stub_request(:put, "#{OctopusClient::BASE_URL}/dossiers/42/buysellbookings")
    ...
    it "returns error on API failure" do
      ...
      stub_request(:put, "#{OctopusClient::BASE_URL}/dossiers/42/buysellbookings")
        .to_return(
          status: 400,
          body: { errorMessage: "Invalid booking data" }.to_json,
